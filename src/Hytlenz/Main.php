...
class Main extends PluginBase implements Listener{
	...
	public function modalForm($player){
        	...
        	$form->setContent(implode("\n", $this->tags($player, $this->greet->getNested("form-message.content"))));
	...
	public function greetings($player){
		if($this->greet->getNested("welcome-message.enable", true)){
			$player->sendMessage(implode("\n", $this->tags($player, $this->greet->getNested("welcome-message.message"))));
		}
		...
		if($this->greet->getNested("effect.enable", true)){
			...
			$player->addEffect(new EffectInstance($effectID, $duration * 20, $amplifier, true));
       		}
